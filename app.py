...
def fetch_yf_data_fixed(ticker_symbol, period="6m", interval="1d"):
    """修復版 yfinance 抓取：解決 MultiIndex 與一維陣列轉換問題"""
    try:
        df = yf.download(ticker_symbol, period=period, interval=interval, auto_adjust=False, progress=False)
        if df.empty:
            return None
        
        # 強制修正 MultiIndex 欄位結構
        ...
        # 展平數值 Series 防止陣列維度錯誤
        df["Close"] = pd.Series(df["Close"].values.flatten(), index=df.index)
        df["Open"] = pd.Series(df["Open"].values.flatten(), index=df.index)
...
if page == "🚀 5日波段選股":
    ...
    def run_5day_strategy(stock_dict):
        ...
        for symbol, name in stock_dict.items():
            try:
                hist_df = fetch_yf_data_fixed(f"{symbol}.TW", period="6m", interval="1d")
...
# ==========================================
    ...
    def fetch_latest_twse_fund():
        """使用 Session 與防護標頭，回溯尋找最近開盤日的籌碼數據"""
    ...
    def check_technical_signals(stock_code):
        try:
            hist = fetch_yf_data_fixed(f"{stock_code}.TW", period="3m", interval="1d")
...
# ==========================================
    ...
    def get_stock_data_with_ta(ticker_symbol):
        df = fetch_yf_data_fixed(ticker_symbol, period="6m", interval="1d")
...
# ==========================================
    ...
    def check_reversal_pattern(df):
        ...
        info = {
            "date": yesterday.name.strftime('%Y-%m-%d') if hasattr(yesterday.name, 'strftime') else str(yesterday.name),
    ...
    if st.button("開始掃描轉折股", type="primary"):
        ...
        with st.spinner("正在抓取行情資料中..."):
            for ticker in stock_list:
                df = fetch_yf_data_fixed(ticker, period="60d", interval="1d")
                if df is None:
                    continue
...
# ==========================================
    ...
    if df_official is None or len(df_official) < 10:
        ...
        df_official['MA5'] = df_official['Close'].rolling(5).mean()
        df_official['MA20'] = df_official['Close'].rolling(20).mean()
        ...
        fig.add_trace(go.Bar(x=df_chart.index, y=df_chart['Volume']/1000, marker_color=colors_vol, name='成交量(張)'), row=2, col=1)
