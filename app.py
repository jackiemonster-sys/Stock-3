...
# ==========================================
    ...
    if stock_id:
        ...
        if df_official is None or len(df_official) < 10:
            ...
            st.subheader(f"📊 股票代號 {stock_id} 核心行情")
            ...
            fig.add_trace(go.Candlestick(x=df_chart['Date'], open=df_chart['Open'], high=df_chart['High'], low=df_chart['Low'], close=df_chart['Close'], name='K線'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df_chart['Date'], y=df_chart['MA5'], name='MA5'), row=1, col=1)
            fig.add_trace(go.Scatter(x=df_chart['Date'], y=df_chart['MA20'], name='MA20'), row=1, col=1)
