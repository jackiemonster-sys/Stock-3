...
# ==========================================
    ...
    if not selected_conditions:
        ...
        if selected_stock:
            ...
            added_labels = set()
            for div_type, occurrences in details.items():
                # 看多/底背離用半透明綠色，看空/頂背離用半透明紅色
                is_bullish = "底背離" in div_type
                bg_color = "rgba(46, 204, 113, 0.25)" if is_bullish else "rgba(231, 76, 60, 0.25)"
                border_color = "#2ecc71" if is_bullish else "#e74c3c"

                for occ in occurrences:
                    s_date = occ['start_date']
                    e_date = occ['end_date']

                    # 避免標籤重複顯示
                    label = div_type if div_type not in added_labels else None
                    if label:
                        added_labels.add(div_type)
